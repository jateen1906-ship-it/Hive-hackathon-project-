...
import { Plus, Truck, ChevronRight, QrCode } from "lucide-react";
            ...
            <Table>
              <TableHeader className="bg-slate-50/80 border-b border-slate-100">
                <TableRow className="border-slate-100 hover:bg-transparent">
                  ...
                  <TableHead className="text-xs font-bold uppercase tracking-wider text-slate-500 py-3.5">Risk Score</TableHead>
                  <TableHead className="text-xs font-bold uppercase tracking-wider text-slate-500 py-3.5 pr-6 text-right">Actions</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody className="divide-y divide-slate-100">
                {data.map((t) => (
                  ...
                  >
                    <TableCell className="py-3.5 pl-6 cursor-pointer" onClick={() => navigate(`/trips/${t.id}`)}>
                      <div className="flex items-center gap-2.5">
                        <RouteStrip origin={t.origin} destination={t.destination} className="text-sm font-semibold" />
                        {t.is_demo && <SyntheticBadge />}
                      </div>
                    </TableCell>
                    <TableCell className="text-xs text-slate-600 py-3.5 font-medium cursor-pointer" onClick={() => navigate(`/trips/${t.id}`)}>
                      {fmtDate(t.travel_date)}
                    </TableCell>
                    <TableCell className="font-mono text-xs text-slate-700 py-3.5 cursor-pointer" onClick={() => navigate(`/trips/${t.id}`)}>
                      {t.vehicle_number || "—"}
                    </TableCell>
                    <TableCell className="text-xs py-3.5 cursor-pointer" onClick={() => navigate(`/trips/${t.id}`)}>
                      <span className="capitalize px-2.5 py-0.5 rounded-full bg-slate-100 text-xs font-medium text-slate-700">
                        {t.status}
                      </span>
                    </TableCell>
                    <TableCell className="py-3.5 cursor-pointer" onClick={() => navigate(`/trips/${t.id}`)}>
                      {t.risk_level ? (
