      ...
      <form onSubmit={submit} className="space-y-6">
        ...
        <Card className="executive-card p-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-1.5">
              <Label htmlFor="origin" className="text-xs text-slate-600">Origin City / Hub *</Label>
            ...
            <div className="space-y-1.5">
              <Label htmlFor="destination" className="text-xs text-slate-600">Destination City / Hub *</Label>
            ...
            <div className="space-y-1.5">
              <Label htmlFor="travel_date" className="text-xs text-slate-600">Dispatch Date</Label>
            ...
            <div className="space-y-1.5">
              <Label htmlFor="declared_distance_km" className="text-xs text-slate-600">E-Way Declared Distance (km)</Label>
          ...
          <div className="mb-5 flex items-center justify-between">
            ...
            <div className="flex gap-1 rounded-lg border border-slate-200 bg-slate-50 p-0.5 text-xs">
              <button type="button" onClick={() => setVehicleMode("new")} data-testid="vehicle-mode-new"
                className={`rounded-md px-3 py-1 font-medium transition-all ${vehicleMode === "new" ? "bg-white text-slate-900 shadow-2xs font-semibold" : "text-slate-500"}`}>New Entry</button>
              <button type="button" onClick={() => setVehicleMode("existing")} data-testid="vehicle-mode-existing"
                className={`rounded-md px-3 py-1 font-medium transition-all ${vehicleMode === "existing" ? "bg-white text-slate-900 shadow-2xs font-semibold" : "text-slate-500"}`}>Saved Fleet</button>
            </div>
          </div>
          {vehicleMode === "existing" && (vehicles || []).length > 0 ? (
            <div className="space-y-1.5">
              <Label className="text-xs text-slate-600">Select fleet vehicle</Label>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-1.5">
                <Label htmlFor="vehicle_number" className="text-xs text-slate-600">Vehicle Registration (RTO)</Label>
              ...
              <div className="space-y-1.5">
                <Label className="text-xs text-slate-600">Carrier Body Type</Label>
        ...
        <Card className="executive-card p-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-1.5 sm:col-span-2">
              <Label htmlFor="goods_description" className="text-xs text-slate-600">Cargo Nature & Description (detects Hazchem/Coldchain/Theft)</Label>
            ...
            <div className="space-y-1.5 sm:col-span-2">
              <Label htmlFor="invoice_value" className="text-xs text-slate-600">Consignment Invoice Value (₹ - GST Rule 138 Check)</Label>
        ...
        <div className="sticky bottom-4 z-20">
          <Button 
            type="submit" 
            size="lg" 
            className="btn-executive-primary w-full font-bold rounded-xl h-11 text-sm shadow-md" 
            disabled={submitting} 
