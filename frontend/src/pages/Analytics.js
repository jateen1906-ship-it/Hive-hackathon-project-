    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-200/80 pb-4">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">Risk distribution and incident heatmap across national corridors.</p>
        </div>
        <div className="flex gap-1 rounded-lg border border-slate-200 bg-slate-100 p-1 text-xs">
          <button
            className={`rounded-md px-3.5 py-1.5 font-semibold transition-all ${period === "all" ? "bg-white text-slate-900 shadow-2xs" : "text-slate-500 hover:text-slate-900"}`}
            onClick={() => setPeriod("all")}
          >All Time</button>
          <button
            className={`rounded-md px-3.5 py-1.5 font-semibold transition-all ${period === "month" ? "bg-white text-slate-900 shadow-2xs" : "text-slate-500 hover:text-slate-900"}`}
          ...
          <Card className="executive-card p-6">
            ...
            {!billing.can.corridorView ? (
              <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-slate-200 bg-slate-50/50 px-6 py-14 text-center" data-testid="corridor-locked">
                ...
                <Button className="btn-executive-primary mt-5 font-semibold text-xs rounded-lg" onClick={() => navigate("/pricing")} data-testid="corridor-upgrade">View Growth & Pro Plans</Button>
              </div>
            ...
            <DialogContent className="max-w-2xl bg-white border-slate-200 text-slate-900">
              ...
              {detail?.data && (
                <div className="max-h-[60vh] space-y-4 overflow-auto">
                  <div>
                    <h4 className="mb-2 text-xs font-bold uppercase tracking-wider text-sky-700">Trips on this corridor ({detail.data.trips.length})</h4>
