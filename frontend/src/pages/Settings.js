...
import { ShieldCheck, Building2, Mail, Phone, Cpu, CreditCard, KeyRound, Copy, Loader2, Trash2, Pencil, Check, X } from "lucide-react";
        ...
        {!isDemo && (
          editing ? (
            <div className="flex gap-1.5">
              <Button size="sm" onClick={() => save.mutate()} disabled={save.isPending} className="btn-primary-blue text-xs h-7">
    ...
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      <Card>
        <div className="flex items-center justify-between border-b border-slate-100 p-5 bg-slate-50/50">
          ...
          {ent && (
            <span className="rounded-full bg-blue-50 border border-blue-200 px-3 py-1 text-xs font-bold text-blue-700" data-testid="settings-current-plan">
          ...
          <div className="p-6">
            <div className="grid gap-3 sm:grid-cols-3">
              <div className="rounded-lg bg-slate-50 border border-slate-100 p-3">
                <div className="text-[11px] text-slate-500 font-medium">Monthly Checks</div>
              ...
              <div className="rounded-lg bg-slate-50 border border-slate-100 p-3">
                <div className="text-[11px] text-slate-500 font-medium">Active Share Links</div>
              ...
              <div className="rounded-lg bg-slate-50 border border-slate-100 p-3">
                <div className="text-[11px] text-slate-500 font-medium">Renewal Cycle</div>
            ...
            <div className="mt-5 flex flex-wrap gap-2.5">
              <Button onClick={() => navigate("/pricing")} data-testid="settings-upgrade" className="btn-primary-blue font-semibold text-xs rounded-lg h-9 px-4">
                {ent.plan === "pro" ? "View Tiers" : "Upgrade Plan"}
              </Button>
              {ent.plan !== "free" && (
                <Button variant="outline" onClick={() => cancel.mutate()} disabled={cancel.isPending} data-testid="settings-cancel-plan" className="border-slate-200 text-xs font-semibold text-red-600 hover:bg-red-50 h-9">
  ...
  return (
    <Card>
      <div className="flex items-center justify-between border-b border-slate-100 p-5 bg-slate-50/50">
        ...
        <Button size="sm" onClick={() => create.mutate()} disabled={create.isPending} data-testid="settings-create-apikey" className="btn-primary-blue font-semibold text-xs h-8">
