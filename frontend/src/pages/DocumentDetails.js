    ...
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-200 pb-4">
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => DocumentAPI.download(id, doc.file_name)} data-testid="document-download" className="border-slate-200 text-xs font-semibold text-slate-700 hover:bg-slate-50 h-8">
            <Download className="mr-1.5 h-3.5 w-3.5" />Download Original
          </Button>
          {!ocrError && (
            <Button variant="outline" onClick={() => validate.mutate()} disabled={validate.isPending} data-testid="document-revalidate" className="border-slate-200 text-xs font-semibold text-slate-700 hover:bg-slate-50 h-8">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
          ...
          <div className="flex items-center justify-between border-b border-slate-100 p-4 bg-slate-50/50">
            ...
            {!ocrError && (editing ? (
              <div className="flex gap-1.5">
                <Button size="sm" onClick={() => save.mutate()} disabled={save.isPending || Object.keys(edits).length === 0} data-testid="document-fields-save" className="btn-primary-blue text-xs h-7">
            ...
            {Object.keys(FIELD_LABELS).map((k) => {
              ...
              return (
                <div key={k} className={`flex items-center justify-between gap-3 px-4 py-2.5 ${low ? "bg-amber-50/70" : ""}`}>
                  <span className="text-xs text-slate-500 font-medium">{FIELD_LABELS[k]}</span>
                  {editing ? (
        ...
        <div className="space-y-4">
          ...
          {!val && !ocrError && <Card className="p-5 text-xs text-slate-500">No pre-check evaluated yet.</Card>}
        </div>
      </div>
              ...
              return (
                <Card key={p.page} className="p-4" data-testid={`document-page-${p.page}`}>
                  <div className="mb-2 flex items-center justify-between">
                    ...
                    <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] text-slate-600 font-medium">{detected.length} field{detected.length !== 1 ? "s" : ""}</span>
                  </div>
                  {detected.length === 0 ? (
